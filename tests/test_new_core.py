...
def test_application_memory_plugin_uses_configured_repository(tmp_path) -> None:
    app = ZyronApplication(Settings(database_path=tmp_path / "zyron.db"))
    response = asyncio.run(app.process_text("lembrar linguagem=python"))
    item = asyncio.run(app.memory_service.recall("linguagem"))
