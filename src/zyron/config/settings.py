...
class Settings:
    ...
    def from_env(cls) -> Settings:
        settings = cls(
            ...
            vosk_model_path=Path(
                _get_string(
                    "ZYRON_VOSK_MODEL_PATH",
                    default="models/vosk-pt",
                )
            ),
    ...
    def validate(self) -> None:
        if not self.assistant_name.strip():
            raise ValueError(
                "O nome do assistente não pode estar vazio."
            )
        ...
        if not self.ollama_base_url.startswith(
            ...
            raise ValueError(
                "A URL do Ollama deve começar com http:// ou https://."
            )
        ...
        if self.audio_channels != 1:
            raise ValueError(
                "O reconhecimento atual exige áudio em canal mono."
            )

        if self.audio_dtype != "int16":
            raise ValueError(
                "O reconhecimento atual exige o formato int16."
            )

        if not self.wake_words:
            raise ValueError(
                "Pelo menos uma palavra de ativação deve ser configurada."
            )
