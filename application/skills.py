...
class SkillMatcher:
    def __init__(self, registry: SkillRegistry) -> None: self._registry = registry
    def match(self, text: str) -> SkillMatch | None:
        ...
        for skill in self._registry.all():
            ...
            scores = [self._score(normalized, self._normalize(term)) for term in terms]
    ...
    def _score(self, text: str, term: str) -> float:
        ...
        if term in text: return min(1.0, 0.72 + len(term)/max(len(text),1)*0.28)
...
class IntentMatcher:
    def __init__(self, matcher: SkillMatcher, low_confidence_threshold: float = 0.5) -> None:
        self._matcher = matcher; self._threshold = low_confidence_threshold
